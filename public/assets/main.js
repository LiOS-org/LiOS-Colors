...
function displayColors(colors, append = false) {
  ...
  colors.forEach((color) => {
    ...
    entryDiv.className = ' frosted_background palette-container';
...
function searchColors() {
  ...
  if (query) {
    isFiltering = true;
    filteredColorData = currentColorData.filter(
      (color) => color.name.toLowerCase().includes(query) || color.hex.toLowerCase().includes(query)
    );
  } else {
    isFiltering = false;
    filteredColorData = [];
